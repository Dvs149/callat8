    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            /* if(Carbon::parse( $pick_update_time )->floatDiffInMinutes( $delivery_0 , false) >= 60){
                $tempErrors = ['delivery_0_delivery_time' => ['Delivery time must be greater then']];
                    break;
            } */
            ...
            if ( !empty($tempErrors) ) {
                $validator->errors()->add('delivery_0_delivery_time', 'Delivery time must have one hour difference');
            }
