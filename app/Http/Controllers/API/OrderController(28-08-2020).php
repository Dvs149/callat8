...
class OrderController extends Controller
{
	...
	public function listOrder( Request $request ) {
	    ...
	    $doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with(['driver'])->with('store')->where('user_id',$user_id)->whereNotIn('order_status',['Completed','Delivered']);
	    $doubleGoogleOrderData_completed = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with(['driver'])->with('store')->where('user_id',$user_id)->where('order_status','Completed')->orWhere('order_status','Delivered');
   ...
   {
		$user_id = $request->get('user_id',''); 
	    $doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with(['driver'])->with('store')->where('user_id',$user_id)->where('created_at','>=',Carbon::now()->subdays(6))->whereNotIn('order_status',['Completed','Delivered']);
	    $doubleGoogleOrderData_completed = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with(['driver'])->with('store')->where('user_id',$user_id)->where('created_at','>=',Carbon::now()->subdays(6))->whereIN('order_status',['Completed','Delivered']);
	    return $this->successJson('Sending item list created successfully.',[
