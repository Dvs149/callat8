...
class OrderController extends Controller
{
	...
	public function listOrder( Request $request ) {
		...
		// dd($user_id);
	   $doubleGoogleOrderData = DoubleGoogleOrderModel::with(['orderAddress'])->where('user_id',$user_id)/* ->where('order_type','Pick up') */;
	//    dd($doubleGoogleOrderData->get()->toArray());
	//    dd($doubleGoogleOrderData->where('id',123)->get()->toArray());
	...
	//    $doubleGoogleOrderData = $doubleGoogleOrderData->get()->toArray();
	   return $this->successJson('Sending item list created successfully.',[
		//    'list' => $this->replace_null_with_empty_string($doubleGoogleOrderData->get()->toArray()),
