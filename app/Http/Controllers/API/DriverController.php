...
class DriverController extends Controller
{
    ...
    {
        $doubleGoogleOrderData_active       = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->with('feedback')->where('driver_id', $request->get('driver_id'))->whereNotIn('order_status', ['Completed', 'Delivered', 'Deleted']);
        $doubleGoogleOrderData_completed    = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->with('feedback')->where('driver_id', $request->get('driver_id'))->whereIn('order_status', ['Completed', 'Delivered']);
    ...
    public function driverGetRemainDelivereyOrderList(DriverOrderListControllerRequest $request)
    {
        $doubleGoogleOrderData_active       = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->where('driver_id', $request->get('driver_id'))->where('order_status', ['Out for delivery']);
        // $doubleGoogleOrderData_active       = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->where('order_status', ['Out for delivery']);
        $doubleGoogleOrderData_completed    = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->where('driver_id', $request->get('driver_id'))->whereIn('order_status', ['Completed', 'Delivered']);
        return $this->successJson('Sending item list created successfully.', [
            'list_active' => $doubleGoogleOrderData_active->orderBy('id', 'DESC')->get()->toArray(),
            'list_completed' => $doubleGoogleOrderData_completed->orderBy('id', 'DESC')->get()->toArray(),
        ]);
    ...
    {
        $doubleGoogleOrderData_active       = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->with('feedback')->where('driver_id', $request->get('driver_id'))->where('created_at','>=',Carbon::now()->subdays(6))->whereNotIn('order_status', ['Completed', 'Delivered', 'Deleted']);
        $doubleGoogleOrderData_completed    = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('customer')->with('doubleGoogleOrderPhoto')->with('feedback')->where('driver_id', $request->get('driver_id'))->where('created_at','>=',Carbon::now()->subdays(6))->whereIn('order_status', ['Completed', 'Delivered']);
        return $this->successJson('Sending item list created successfully.', [
            'list_active' => $doubleGoogleOrderData_active->orderBy('id', 'DESC')->get()->toArray(),
            'list_completed' => $doubleGoogleOrderData_completed->orderBy('id', 'DESC')->get()->toArray(),
        ]);
    ...
    public function driverOrderStatus(DriverOrderStatusControllerRequest $request)
    {
	    DB::beginTransaction();
        try{
            $order_status_msg_to_custmr="";
            $orderlist = DoubleGoogleOrderModel::with('customer')->where('id', $request->get('order_id'))->first();
            $orderlist->order_status = $request->get('order_status');
            $orderlist->save();
            // dd($orderlist->customer->sender_mobile);
            if($orderlist->order_status == 'Out for delivery'){
                $order_status_msg_to_custmr="Your order ".$orderlist->order_number." is out for delivery.";
            } else if($orderlist->order_status == "Delivered"){
                $order_status_msg_to_custmr="Your order ".$orderlist->order_number." is delivered.";
            } /* else if($orderlist->order_status == ""){
                $order_status_msg_to_custmr="Your order ".$orderlist->order_number." is out for delivery.";
            ...
            $message = urlencode($order_status_msg_to_custmr);
            $output = $this->send_sms($orderlist->customer->sender_mobile,$message);
    ...
    public function orderStatusList()
    {
        return $this->successJson('Order status fetch successfully.', [
            'order_status' => ['Pending', 'Completed', 'Canceled', 'Out for delivery', 'Delivered', 'Deleted', 'Return','Confirm'],
        ]);
    }
    ...
    public function submitDriverLocation(SubmitDriverLocationControllerRequest $request)
    {
        ...
        DB::beginTransaction();
        try {
            $order_string=$request->get('order_id');
            $order_array = array_values(array_filter(explode(",",$order_string)));
            // dd($order_array);
			foreach( $order_array as $key => $value ) {
                    $driver_location = DriverLocation::updateOrCreate([
                        'driver_id' => $request->get('driver_id'),
                        'double_google_order_id' => $value,
                    ],[
    ...
    public function driverGetOrderDetail(Request $request)
    {
            $doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['customer'])->with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['feedback'])->where('id', $request->order_id);
            return $this->successJson('Order list fetch successfully.', [
                'order_details' => $doubleGoogleOrderData_active->first()->toArray(),
            ]);
    }
}
