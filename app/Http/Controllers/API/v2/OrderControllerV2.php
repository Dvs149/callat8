...
class OrderControllerV2 extends Controller
{
    ...
    public function saveV2( OrderControllerRequest $request ) {
    	...
    	try {
			...
			if($request->get('order_type')=="Pick up" && $request->get('user_id')==null ) {
				...
				foreach( $request->get('delivery') as $key => $value ) {
					...
					$cust_address=$value['delivery_address'];
				}
			}  
			...
			if($request->get('user_id')!=null){
				...
				$device_detail = UserDeviceDetail::updateOrCreate([
                    ...
                    'fcm_key' => $request->get('fcm_key',null),
                    'device_type' => $request->get('device_type',null),
                    'device_version' => $request->get('device_version',null),
                    'manfacturer' => $request->get('manfacturer',null),
			...
			$count_of_address = AddAddressModel::where('user_id',$customer->id)->count();
			if($count_of_address == 0){
				if($request->get('address_list_id')==0){
					if( $request->get('order_type')=="Pick up" ){
						$addAddress = AddAddressModel::create([
							'address' => $request->get('pickup_address',''),
							'address_type' => 'Home',
							'user_id' => $customer->id,
							'address_lat' => $request->get('pickup_lat','0.00'),
							'address_long' => $request->get('pickup_long','0.00'),
							'default_address' => 'Y',
						]);
					} else {
						foreach( $request->get('delivery') as $key => $value ) {
							$addAddress = AddAddressModel::create([
								...
								'address_type' => 'Home',
								'user_id' => $customer->id,
								'address_lat' => isset($value['delivery_lat']) ? $value['delivery_lat'] : '0.00',
								'address_long' => isset($value['delivery_long']) ? $value['delivery_long'] : '0.00',
								'default_address' => 'Y',
							]);				
						}
					}
	    	...
	    	$returnDoubleGoogleOrder = DoubleGoogleOrderModel::updateOrCreate([
				...
				'version_code' => $request->get('version_code',null),
				'user_id' => $customer->id,
			...
			if($returnDoubleGoogleOrder->version_code==null){
				$orderTrackingLog_body['user_id'] = $request->get('admin_id',null);
				$orderTrackingLog_body['created_on'] = 'Web';
			} else {
				$orderTrackingLog_body['user_id'] = $customer->id;
				$orderTrackingLog_body['created_on'] = 'App';
			}

			$orderTrackingLog_body['order_id'] = $returnDoubleGoogleOrder->id;
            $orderTrackingLog_body['order_status'] = $returnDoubleGoogleOrder->order_status;
            // $orderTrackingLog_body['created_at'] = $returnDoubleGoogleOrder->created_at;
			...
			if(!empty($returnDoubleGoogleOrderItem)){
				...
				if( $request->get('notify_me_by_sms')=='Yes' && $request->get('order_type')=="Pick up"){
					$user_message="Thank you for your order ".$order_number." on callat7, We will update you after your order is delivered from our end.";
					$message = urlencode($user_message);
					$output = $this->send_sms($customer->sender_mobile,$message);
				}

				if( $request->get('notify_recipients_by_sms')=='Yes' && $request->get('order_type')=="Pick up"){

						foreach( $request->get('delivery') as $key => $value ) {
							$user_message=$customer->sender_name." has send you a parcel. Order number is ".$order_number.". Thank you callat7";
							$message = urlencode($user_message);
							$output = $this->send_sms($value['delivery_phone'],$message);
						}
				}
