...
use App\Http\Requests\API\v1\order\GetOrderDetailControllerRequest;
...
class OrderController extends Controller
{
    ...
    public function save( OrderControllerRequest $request ) {
    	...
    	try {
	    	...
	    	if( $returnDoubleGoogleOrder && $request->get('order_type')=="Pick up") {
			...
			} else if($returnDoubleGoogleOrder && ($request->get('order_type')=="Store" || $request->get('order_type')=='Anything else')){
	    		...
	    		foreach( $request->get('delivery') as $key => $value ) {
	    			$returnOrderAddress = OrderAddressModel::create([
						...
						'pickup_long'=> $request->get('pickup_long','0.00'),
						'delivery_address' => isset($value['delivery_address']) ? $value['delivery_address'] : null,
						'delivery_phone' => $request->get('pickup_phone'),
						// isset($value['delivery_phone']) ? $value['delivery_phone'] : null,
						'delivery_date' => isset($value['delivery_date']) ? date('Y-m-d',strtotime($value['delivery_date'])) : null,
						'delivery_time' => isset($value['delivery_time']) ? date('H:i:s',strtotime($value['delivery_time'])) : null,
                        'delivery_distance' => isset($value['delivery_distance']) ? $value['delivery_distance'] : '0.00',
						'delivery_comment' => isset($value['delivery_comment']) ? $value['delivery_comment'] : null,
	...
	public function listOrder( Request $request ) {
	    ...
	    $doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->whereNotIn('order_status',['Completed','Delivered']);
	    $doubleGoogleOrderData_completed = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->whereIn('order_status',['Completed','Delivered']);
	...
	public function getOrderDetail(GetOrderDetailControllerRequest $request)
	{
		$doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->with(['feedback'])->where('id',$request->order_id);
   ...
   public function deleteOrder(DeleteOrderControllerRequest $request)
	{	
		...
		try{
			...
			$orderTrackingLog_body['user_id'] = $customer->id;
			$orderTrackingLog_body['order_id'] = $orderlist->id;
			$orderTrackingLog_body['order_status'] = $orderlist->order_status;
			// $orderTrackingLog_body['created_at'] = $orderlist->created_at;
			$orderTrackingLog_body['created_on'] = 'App';
			// $orderTrackingLog_body['driver_id'] = 
   ...
   public function checkDriverDetails(OrderDriverDetailsControllerRequest $request)
   {
		...
		$driver_details = Driver::with(['driverLocation' => function($q) use($order) {
											$q->where('double_google_order_id', $order->id)->first();
										}])->where('id',$order->driver_id)->first();
		// dd($driver_details->toArray());
		return $this->successJson('Driver fetch succesfully',[
			'driver_details' => empty($driver_details) ? null : $driver_details->toArray(),
		]);
	}
	 
	public function last7DaysOrder(Request $request)
   {
		$user_id = $request->get('user_id',''); 
	    // $doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->where('created_at','>=',Carbon::now()->subdays(6))->whereNotIn('order_status',['Completed','Delivered']);
		// $doubleGoogleOrderData_completed = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->where('created_at','>=',Carbon::now()->subdays(6))->where('order_status', 'Completed')->orWhere('order_status', 'Delivered');
		$doubleGoogleOrderData_active = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->whereNotIn('order_status',['Completed','Delivered']);
	    $doubleGoogleOrderData_completed = DoubleGoogleOrderModel::with(['orderAddress'])->with(['doubleGoogleOrderItems'])->with('doubleGoogleOrderPhoto')->with(['driver'])->where('user_id',$user_id)->whereIN('order_status', ['Completed','Delivered']);
	    return $this->successJson('Sending item list created successfully.',[
